# Alzheimer's Detection from Brain MRI (2D CNN)

A deep learning system that detects Alzheimer's disease from brain MRI scans. It slices 3D MRI volumes into 2D images, classifies them with a convolutional neural network, and is evaluated under a strict patient-level split to give an honest estimate of real-world performance.

The project is as much about **rigorous evaluation** as it is about the model. Two issues that commonly inflate medical-imaging results were identified and addressed here: patient-level data leakage and overfitting. The numbers reported are the corrected, honest ones.
...
Alzheimer's disease produces subtle, diffuse structural changes in the brain (atrophy, enlarged ventricles) rather than a single discrete lesion. This makes it a genuinely hard classification target from MRI — far harder than, say, detecting a visible tumor mass.
...
4. Reports honest metrics — including a clear-eyed account of where the model is weak.
...
An early version split the data by individual slice. Because many slices come from the same patient, slices from one brain leaked into both training and validation — letting the model recognize patients rather than disease, and inflating accuracy. The pipeline now splits strictly **by patient** using `GroupShuffleSplit` on patient IDs, with an assertion that guarantees zero patient overlap between splits. Every number here is on this clean split.
...
3. **Patient-level split** — slices are grouped by patient ID, then split by patient so no individual appears in both training and validation.
...
6. **Evaluation** — confusion matrix, per-class precision/recall, sensitivity, specificity, and ROC-AUC on the held-out patients.
...
- **Output:** two class logits (Normal / Alzheimer's).
...
├── assets/                       # Grad-CAM / dashboard images
...
- **2D single-slice modeling** — Alzheimer's atrophy is a 3D pattern; a 3D CNN or multi-slice input would capture volumetric structure a single slice misses, and is the most promising route to a higher ceiling.
